...
if [ ${ENV} = "development" ] || [ ${ENV} = "release-candidates" ]; then
    ...
    if [ -d "/tmp/${USER}-log-push" ]; then
        echo "Folder /tmp/${USER}-log-push already exist"
    else
        mkdir /tmp/${USER}-log-push
    fi
else
    LOG="/tmp/[ENV]-${SERVICE}-log-push/${DATELOG}.txt"
    if [ -d "/tmp/[ENV]-${SERVICE}-log-push" ]; then
        echo "Folder /tmp/[ENV]-${SERVICE}-log-push already exist"
    else
        mkdir /tmp/[ENV]-${SERVICE}-log-push
    fi
fi
